...
def plot_circle_radius(data, source):
    ...
    plot_line_circle.title.text = "Circle Radius Label num >= 3"
...
width = data_dict['data6']['image'][0].shape[0]
height = data_dict['data6']['image'][0].shape[1]

x = list(range(start, start + len(data_dict['data6']['circle_radius'])))
...
circle_line_plot = plot_circle_radius(
    data=data_dict['data6'], source=source_dict['source6'])
...
plot_column = column(circle_line_plot, ellipse_dict['ellipse6'])

layout_frame = column(
    row(figure_dict['figure6'], plot_column),
    slider
)
