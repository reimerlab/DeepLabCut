...
case = '20892_9_00010_compressed_cropped_{}_{}_ellipse_relaxed'.format(
    start, end)
...
def update(attr, old, new):
    ...
    source.data = dict(image=[data['image'][slider.value-start]],
                       circle_radius=['nan' if np.isnan(
                           data['circle_radius'][slider.value-start]) else data['circle_radius'][slider.value-start]],
                       major_radius=['nan' if np.isnan(
                           data['major_radius'][slider.value-start]) else data['major_radius'][slider.value-start]],
                       minor_radius=['nan' if np.isnan(
                           data['minor_radius'][slider.value-start]) else data['minor_radius'][slider.value-start]],
                       frame_num=[x[slider.value-start]],
                       circle_radius_text=["{:.2f}".format(
                           data['circle_radius'][slider.value-start])],
                       major_radius_text=["{:.2f}".format(
                           data['major_radius'][slider.value-start])],
                       minor_radius_text=["{:.2f}".format(
                           data['minor_radius'][slider.value-start])]
                       )
    plot_fig.title.text = "Frame num: {}".format(slider.value)
...
layout_frame = column(
    gridplot([
        [plot_fig, column(plot_line_circle, plot_line_ellipse)]
        ]), slider)
